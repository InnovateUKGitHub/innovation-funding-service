...
public class HorizonWorkProgrammeController {
    ...
    private List<HorizonWorkProgramme> parentsForWorkflow = new ArrayList<>();
    ...
    public String submitWorkProgramme(@Valid @ModelAttribute(value = "form") HorizonWorkProgrammeForm form,
        ...
        List<HorizonWorkProgramme> selectLocation = Collections.singletonList(form.getSelected());
        ...
        return validationHandler.failNowOrSucceedWith(failureView, () -> {
            ...
            if (this.parentsForWorkflow.isEmpty()) {
                setParentWorkflow(selectLocation);
            }

            return redirect(applicationId, questionId);
        });
    }
    ...
    public String viewCallIds(@ModelAttribute(value = "form") HorizonWorkProgrammeForm form,
        ...
        Optional<HorizonWorkProgrammeSelectionData> horizonWorkProgrammeSelectionData = cookieService.getHorizonWorkProgrammeSelectionData(request);
        HorizonWorkProgramme existingSelection = horizonWorkProgrammeSelectionData
                .map(HorizonWorkProgrammeSelectionData::getCallId)
                .orElse(null);

        form.setAllOptions(HorizonWorkProgramme.findChildrenOf(horizonWorkProgrammeSelectionData.get().getWorkProgramme()));
        form.setSelected(existingSelection);
    ...
    private String redirect(long applicationId, long questionId) {
        return "redirect:" + APPLICATION_BASE_URL + applicationId + "/form/question/" + questionId + "/horizon-work-programme" + "/call-id";
    }

    private void setParentWorkflow(List<HorizonWorkProgramme> selectedParentLocations) {
        this.parentsForWorkflow = selectedParentLocations
                .stream()
                .filter(location -> !HorizonWorkProgramme.findChildrenOf(location).isEmpty())
    ...
    private Map<String, List<HorizonWorkProgramme>> getReadOnlyMap(long applicationId, HttpServletRequest request) {
        ...
        Optional<HorizonWorkProgrammeSelectionData> projectLocationSelectionData = cookieService.getHorizonWorkProgrammeSelectionData(request);
        List<HorizonWorkProgramme> savedSelections = (projectLocationSelectionData.isPresent() && !projectLocationSelectionData.get().getAllSelections().isEmpty()) ?
