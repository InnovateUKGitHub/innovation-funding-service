...
public class BaseUnitTest {
    ...
    public void setupCompetition(){
        ...
        QuestionResourceBuilder questionResourceBuilder = newQuestionResource().with(competition(competitionResource.getId()));

        SectionResourceBuilder sectionResourceBuilder = newSectionResource().with(competition(competitionResource.getId()));
    ...
    public void setupApplicationWithRoles(){
        ...
        // TODO BO - remove temporary Organisation and ProcessRoles below when Assessment is converted to DTO
        Organisation temporaryOrganisation1 = OrganisationBuilder.newOrganisation().withId(organisation1.getId()).withName("Empire Ltd").withOrganisationType(businessOrganisationType).build();
        ProcessRole temporaryProcessRole6 = ProcessRoleBuilder.newProcessRole().with(id(6L)).withApplication(applicationList.get(1)).withUser(assessorUser).withRole(assessorRole).withOrganisation(temporaryOrganisation1).build();
        ProcessRole temporaryProcessRole7 = ProcessRoleBuilder.newProcessRole().with(id(7L)).withApplication(applicationList.get(2)).withUser(assessorUser).withRole(assessorRole).withOrganisation(temporaryOrganisation1).build();
        ProcessRole temporaryProcessRole8 = ProcessRoleBuilder.newProcessRole().with(id(8L)).withApplication(applicationList.get(0)).withUser(assessorUser).withRole(assessorRole).withOrganisation(temporaryOrganisation1).build();
        ProcessRole temporaryProcessRole9 = ProcessRoleBuilder.newProcessRole().with(id(9L)).withApplication(applicationList.get(3)).withUser(assessorUser).withRole(assessorRole).withOrganisation(temporaryOrganisation1).build();
        assessorProcessRolesTemporary = asList(temporaryProcessRole6, temporaryProcessRole7, temporaryProcessRole8, temporaryProcessRole9);
    ...
    public void setupApplicationResponses(){
        ...
        ResponseResource responseResource = new ResponseResource(1L, LocalDateTime.now(), userApplicationRole.getId(), 20L, app);
        ResponseResource responseResource2 = new ResponseResource(2L, LocalDateTime.now(), userApplicationRole.getId(), 21L, app);
