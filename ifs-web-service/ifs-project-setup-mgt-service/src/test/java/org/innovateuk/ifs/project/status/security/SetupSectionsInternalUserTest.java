...
public class SetupSectionsInternalUserTest extends BaseUnitTest {
    ...
    public void testCheckAccessToMonitoringOfficerSectionHappyPath() {
        ...
        assertEquals(ACCESSIBLE, internalUser.canAccessMonitoringOfficerSection(newUserResource().withRolesGlobal(asList(Role.COMP_ADMIN)).build()));
    ...
    private UserResource getSupportUser(){
        return newUserResource().withRolesGlobal(asList(Role.SUPPORT)).build();
    }

    private UserResource getInnovationLeadUser(){
        return newUserResource().withRolesGlobal(asList(Role.INNOVATION_LEAD)).build();
    }

    @Test
    public void testSupportUserCanAccessIfMonitoringOfficerSubmitted() {
        UserResource supportUser = newUserResource().withRolesGlobal(asList(Role.SUPPORT)).build();
    ...
    public void testInnovationLeadUserCanAccessIfMonitoringOfficerSubmitted() {
        UserResource supportUser = newUserResource().withRolesGlobal(asList(Role.INNOVATION_LEAD)).build();
    ...
    public void testCheckAccessToBankDetailsSectionButNotFinanceTeamMember() {
        ...
        stream(UserRoleType.values()).forEach(role -> {
            if (role != PROJECT_FINANCE) {
                ...
                List<Role> roles = asList(Role.getByName(role.getName()));
    ...
    public void testCheckAccessToSpendProfileSectionHappyPath() {
        ...
        assertEquals(ACCESSIBLE, internalUser.canAccessSpendProfileSection(newUserResource().withRolesGlobal(asList(Role.COMP_ADMIN)).build()));
        verifyInteractions(
    ...
    private UserResource getFinanceTeamMember() {
        List<Role> roles = asList(Role.PROJECT_FINANCE);
    ...
    public void testCheckAccessToOtherDocumentsSectionHappyPath() {
        ...
        assertEquals(ACCESSIBLE, internalUser.canAccessOtherDocumentsSection(newUserResource().withRolesGlobal(asList(Role.COMP_ADMIN)).build()));
    ...
    public void testCheckAccessToGrantOfferLetterSendSectionHappyPath() {
        ...
        List<Role> roles = asList(Role.COMP_ADMIN);
