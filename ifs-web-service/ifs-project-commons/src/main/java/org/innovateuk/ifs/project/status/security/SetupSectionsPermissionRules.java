...
public class SetupSectionsPermissionRules {
    ...
    public boolean internalAdminUserCanApproveDocuments(ProjectCompositeId projectCompositeId, UserResource user) {
        return isMOJourneyUpdateEnabled ? (isInternalAdmin(user) || hasIFSAdminAuthority(user) || isMonitoringOfficer(user)) : (isInternalAdmin(user) || hasIFSAdminAuthority(user));
    }
