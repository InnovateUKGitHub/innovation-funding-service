...
public class YourProjectLocationPopulator extends AbstractSectionPopulator<YourProjectLocationSectionViewModel> {
    ...
    protected void populateNoReturn(ApplicantSectionResource section, ApplicationForm form, YourProjectLocationSectionViewModel viewModel, Model model, BindingResult bindingResult, Boolean readOnly, Optional<Long> applicantOrganisationId) {
        ...
        String projectLocation = applicationFinanceRestService.getApplicationFinance(viewModel.getApplication().getId(), viewModel.getApplicantOrganisationId()).getSuccess().getProjectLocation();
