...
public abstract class AbstractSectionPopulator<M extends AbstractSectionViewModel> extends BaseModelPopulator {
    ...
    public M populate(ApplicantSectionResource section, ApplicationForm form, Model model) {
        M viewModel = createNew(section, form);
        populate(section, form, viewModel, model);
        return viewModel;
    }

    protected abstract void populate(ApplicantSectionResource section, ApplicationForm form, M viewModel, Model model);
