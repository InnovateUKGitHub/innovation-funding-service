...
public class PasswordPolicyValidator {
    ...
    void postConstruct() {
        ...
        ExclusionRule containsOrganisationName = new ExclusionRule(PASSWORD_MUST_NOT_CONTAIN_ORGANISATION_NAME, 0, false, user -> {
        ...
        ExclusionRulePatternGenerator lettersForNumbersGenerator = new ExclusionRulePatternGenerator() {
    ...
    private List<List<String>> findPermutations(List<String> excludedWords) {
        List<List<List<String>>> allPermutations = mapWithIndex(excludedWords, (i, currentWord) -> findPermutations(emptyList(), currentWord, removeElement(excludedWords, i)));
        return flattenLists(allPermutations);
    }

    private List<List<String>> findPermutations(List<String> permutationStringSoFar, String currentWord, List<String> remainingWords) {

        List<String> newPermutationStringSoFar = combineLists(permutationStringSoFar, currentWord);

        if (remainingWords.isEmpty()) {
            return singletonList(newPermutationStringSoFar);
        }

        List<List<List<String>>> furtherPermutations = mapWithIndex(remainingWords, (i, remainingWord) -> findPermutations(newPermutationStringSoFar, remainingWord, removeElement(remainingWords, i)));
