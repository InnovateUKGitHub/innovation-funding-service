...
public class AlertController {
    ...
    public RestResult<List<AlertResource>> findAllVisibleByType(@PathVariable("type") final AlertType type) {
        return alertService.findAllVisibleByType(type).toGetResponse();
    }

    @RequestMapping(value= "/{id}", method = GET)
    public RestResult<AlertResource> findById(@PathVariable("id") final Long id) {
        return alertService.findById(id).toGetResponse();
    }

    @RequestMapping(value = "/", method = POST)
    public RestResult<AlertResource> create(@RequestBody final AlertResource alertResource) {
    ...
    public RestResult<Void> deleteAllByType(@PathVariable("type") final AlertType type) {
