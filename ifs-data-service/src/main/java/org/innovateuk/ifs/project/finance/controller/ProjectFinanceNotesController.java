...
public class ProjectFinanceNotesController {
    ...
    public RestResult<List<Note>> notes(@PathVariable("projectFinanceId") final Long projectFinanceId) {
        return service.findAll(projectFinanceId).toGetResponse();
    }

    @RequestMapping(value = "", method = POST)
    public RestResult<Void> createNote(@RequestBody Note note ) {
        return service.create(note).toPostCreateResponse();
    }

    @RequestMapping(value = "/{noteId}/post", method = POST)
    public RestResult<Void> addPost(@PathVariable("noteId") Long noteId, @RequestBody Post post ) {
