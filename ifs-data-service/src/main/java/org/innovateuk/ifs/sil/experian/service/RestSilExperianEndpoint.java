...
public class RestSilExperianEndpoint implements SilExperianEndpoint {
    ...
    public ServiceResult<ValidationResult> validate(SILBankDetails bankDetails) {
        final Either<ResponseEntity<SilError>, ResponseEntity<ValidationResultWrapper>> response = adaptor.restPostWithEntity(silExperianValidate, bankDetails, ValidationResultWrapper.class, SilError.class, HttpStatus.OK, HttpStatus.ACCEPTED);
        if(response.isLeft()){
    ...
    public ServiceResult<VerificationResult> verify(AccountDetails accountDetails) {
        final Either<ResponseEntity<SilError>, ResponseEntity<VerificationResultWrapper>> response = adaptor.restPostWithEntity(silExperianVerify, accountDetails, VerificationResultWrapper.class, SilError.class, HttpStatus.OK, HttpStatus.ACCEPTED);
        if(response.isLeft()){
