...
public class ProjectFinanceQueriesServiceSecurityTest extends BaseServiceSecurityTest<ProjectFinanceQueriesService> {
    ...
    public void test_findOne() throws Exception {
        ...
        assertAccessDenied(() -> classUnderTest.findOne(1L), () -> {
            verify(queryRules).onlyProjectFinanceUsersOrPartnersCanViewTheirQueries(isA(QueryResource.class), isNull(UserResource.class));
            verifyNoMoreInteractions(queryRules);
        });
    }
    ...
    public void test_findAll() throws Exception {
        ...
        verify(queryRules, times(2)).onlyProjectFinanceUsersOrPartnersCanViewTheirQueries(isA(QueryResource.class), isNull(UserResource.class));
        verifyNoMoreInteractions(queryRules);
    }
    ...
    public void test_addPost() throws Exception {
        ...
        assertAccessDenied(() -> classUnderTest.addPost(isA(PostResource.class), 3L), () -> {
            verify(queryRules).onlyProjectFinanceUsersOrPartnersAddPostToTheirQueries(isA(QueryResource.class), isNull(UserResource.class));
            verifyNoMoreInteractions(queryRules);
        });
    }
