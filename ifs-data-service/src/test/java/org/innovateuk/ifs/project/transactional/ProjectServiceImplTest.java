...
public class ProjectServiceImplTest extends BaseServiceUnitTest<ProjectService> {
    ...
    public void testSendGrantOfferLetterFailure(){
        ...
        when(golWorkflowHandlerMock.grantOfferLetterSent(p, user)).thenReturn(FALSE);
    ...
    public void testIsSendGrantOfferLetterAllowed() {
        ...
        when(projectRepositoryMock.findOne(projectId)).thenReturn(p);
        when(golWorkflowHandlerMock.isSendAllowed(p)).thenReturn(TRUE);
    ...
    public void testIsSendGrantOfferLetterAllowedFails() {
        ...
        when(projectRepositoryMock.findOne(projectId)).thenReturn(p);
        when(golWorkflowHandlerMock.isSendAllowed(p)).thenReturn(FALSE);

        ServiceResult<Boolean> result = service.isSendGrantOfferLetterAllowed(projectId);

        assertTrue(result.isSuccess() && FALSE == result.getSuccessObject());
    }
    ...
    public void testApproveSignedGrantOfferLetterSuccess(){
        ...
        when(golWorkflowHandlerMock.isReadyToApprove(p)).thenReturn(TRUE);
        when(userRepositoryMock.findOne(u.getId())).thenReturn(u);
        when(golWorkflowHandlerMock.grantOfferLetterApproved(p, u)).thenReturn(TRUE);
        when(projectWorkflowHandlerMock.grantOfferLetterApproved(p, p.getProjectUsersWithRole(PROJECT_MANAGER).get(0))).thenReturn(TRUE);
    ...
    public void testApproveSignedGrantOfferLetterFailure(){
        ...
        when(golWorkflowHandlerMock.isReadyToApprove(p)).thenReturn(TRUE);
        when(userRepositoryMock.findOne(u.getId())).thenReturn(u);
        when(golWorkflowHandlerMock.grantOfferLetterApproved(p, u)).thenReturn(FALSE);
    ...
    public void testApproveSignedGrantOfferLetterFailureNotReadyToApprove(){
        ...
        when(projectRepositoryMock.findOne(projectId)).thenReturn(p);
        when(golWorkflowHandlerMock.isReadyToApprove(p)).thenReturn(FALSE);
    ...
    public void testGetSignedGrantOfferLetterApprovalStatusSuccess(){
        ...
        when(projectRepositoryMock.findOne(projectId)).thenReturn(p);
        when(golWorkflowHandlerMock.isApproved(p)).thenReturn(TRUE);
        ...
        assertTrue(result.isSuccess() && TRUE == result.getSuccessObject());
    }
    ...
    public void testGetSignedGrantOfferLetterApprovalStatusFailure(){
        ...
        when(projectRepositoryMock.findOne(projectId)).thenReturn(p);
        when(golWorkflowHandlerMock.isApproved(p)).thenReturn(FALSE);
