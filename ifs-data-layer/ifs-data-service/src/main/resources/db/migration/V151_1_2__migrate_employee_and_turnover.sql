...
update employees_and_turnover e
    ...
    inner join form_input_response turnover_response on turnover_response.form_input_id = turnover_input.id and turnover_response.application_id=app.id
    inner join process_role turnover_updated_by on turnover_updated_by.id = turnover_response.updated_by_id and turnover_updated_by.organisation_id = af.organisation_id
...
update employees_and_turnover e
    ...
    inner join form_input_response employees_response on employees_response.form_input_id = employees_input.id and employees_response.application_id=app.id
    inner join process_role employees_updated_by on employees_updated_by.id = employees_response.updated_by_id and employees_updated_by.organisation_id = af.organisation_id

SET e.employees = NULLIF(employees_response.value, '')
