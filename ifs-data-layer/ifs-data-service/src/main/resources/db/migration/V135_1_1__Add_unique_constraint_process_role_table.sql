...
UPDATE question_status qs
       JOIN process_role delete_role
         ON qs.marked_as_complete_by_id = delete_role.id
       JOIN process_role join_role
         ON join_role.application_id = delete_role.application_id
            AND join_role.user_id = delete_role.user_id
            AND join_role.role_id = delete_role.role_id
            AND join_role.id != delete_role.id
SET    qs.marked_as_complete_by_id = join_role.id
WHERE  marked_as_complete_by_id IN (SELECT *
                                    FROM   process_roles_to_delete);

-- 4. Merge question_status assigned_by_id into new ids
UPDATE question_status qs
       JOIN process_role delete_role
         ON qs.assigned_by_id = delete_role.id
       JOIN process_role join_role
         ON join_role.application_id = delete_role.application_id
            AND join_role.user_id = delete_role.user_id
            AND join_role.role_id = delete_role.role_id
            AND join_role.id != delete_role.id
SET    qs.assigned_by_id = join_role.id
WHERE  assigned_by_id IN (SELECT *
                          FROM   process_roles_to_delete);

-- 5. Merge question_status assignee_id into new ids
UPDATE question_status qs
       JOIN process_role delete_role
         ON qs.assignee_id = delete_role.id
       JOIN process_role join_role
         ON join_role.application_id = delete_role.application_id
            AND join_role.user_id = delete_role.user_id
            AND join_role.role_id = delete_role.role_id
            AND join_role.id != delete_role.id
SET    qs.assignee_id = join_role.id
WHERE  assignee_id IN (SELECT *
                       FROM   process_roles_to_delete);
...
-- 8. Add unique constraint to process_role rows
