...
import org.innovateuk.ifs.project.monitoring.transactional.MonitoringOfficerInviteServiceImpl;
...
import static org.innovateuk.ifs.acc.AccMonitoringOfficerInviteServiceImpl.Notifications.MONITORING_OFFICER_NEW_PROJECT_NOTIFICATION;
import static org.innovateuk.ifs.acc.AccMonitoringOfficerInviteServiceImpl.Notifications.MONITORING_OFFICER_REGISTRATION_INVITE;
 ...
 */
@Service
public class AccMonitoringOfficerInviteServiceImpl extends InviteService<AccMonitoringOfficerInvite> implements AccMonitoringOfficerInviteService {

    private static final Log LOG = LogFactory.getLog(AccMonitoringOfficerInviteServiceImpl.class);

    private static final String DEFAULT_INTERNAL_USER_EMAIL_DOMAIN = "innovateuk.ukri.org";
    private static final String WEB_CONTEXT = "/management/acc-user";
    ...
    @Autowired
    private AccMonitoringOfficerInviteRepository monitoringOfficerInviteRepository;
    ...
    @Autowired
    private AccMonitoringOfficerInviteMapper monitoringOfficerInviteMapper;
    ...
    private ServiceResult<AccMonitoringOfficerInvite> saveInvite(User invitedUser) {
        AccMonitoringOfficerInvite monitoringOfficerInvite = new AccMonitoringOfficerInvite(
    ...
    private ServiceResult<Void> sendInviteToUnregistered(AccMonitoringOfficerInvite invite,
                                                         Project project) {
    ...
    private static String getInviteUrl(String baseUrl, AccMonitoringOfficerInvite monitoringOfficerInvite) {
        return String.format("%s/%s/%s", baseUrl, monitoringOfficerInvite.getHash(), "register");
    }

    @Override
    public ServiceResult<AccMonitoringOfficerInviteResource> getInviteByHash(String hash) {
        return getByHash(hash).andOnSuccessReturn(monitoringOfficerInviteMapper::mapToResource);
    }

    @Override
    @Transactional
    public ServiceResult<AccMonitoringOfficerInviteResource> openInvite(String hash) {
    ...
    public ServiceResult<User> activateUserByHash(String inviteHash,
                                                  AccUserRegistrationResource resource) {
    ...
    private ServiceResult<User> updateUserWithRegistrationDetails(Invite invite, AccUserRegistrationResource resource) {
    ...
    @Override
    protected Class<AccMonitoringOfficerInvite> getInviteClass() {
        return AccMonitoringOfficerInvite.class;
    }

    @Override
    protected InviteRepository<AccMonitoringOfficerInvite> getInviteRepository() {
        return monitoringOfficerInviteRepository;
    }

    private ServiceResult<User> applyMonitoringOfficerRoleFromInvite(AccMonitoringOfficerInvite invite) {
