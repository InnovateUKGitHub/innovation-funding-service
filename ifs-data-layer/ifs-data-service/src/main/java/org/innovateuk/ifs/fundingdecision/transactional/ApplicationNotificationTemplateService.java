...
public interface ApplicationNotificationTemplateService {
	...
	@SecuredBySpring(value = "GET_DEFAULT_SUCCESSFUL_TEMPLATE", securedType = Decision.class, description = "Comp Admins should be able to see default templates for application emails.")
	ServiceResult<ApplicationNotificationTemplateResource> getSuccessfulNotificationTemplate(long competitionId);

	@PreAuthorize("hasAnyAuthority('comp_admin' , 'project_finance')")
	@SecuredBySpring(value = "GET_DEFAULT_UNSUCCESSFUL_TEMPLATE", securedType = Decision.class, description = "Comp Admins should be able to see default templates for application emails.")
