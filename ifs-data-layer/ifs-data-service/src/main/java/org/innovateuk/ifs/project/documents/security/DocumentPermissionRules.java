...
public class DocumentPermissionRules extends BasePermissionRules {
    ...
    public boolean internalAdminCanApproveDocument(ProjectResource project, UserResource user) {
        return isMOJourneyUpdateEnabled ? (isInternalAdmin(user) || hasIFSAdminAuthority(user) || isMonitoringOfficer(project.getId(), user.getId())) : (isInternalAdmin(user) || hasIFSAdminAuthority(user));
    }
