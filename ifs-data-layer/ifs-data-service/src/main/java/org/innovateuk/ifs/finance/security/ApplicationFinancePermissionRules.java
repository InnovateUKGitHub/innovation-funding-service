...
public class ApplicationFinancePermissionRules extends BasePermissionRules {
    ...
    @PermissionRule(value = "READ", description = "A cofunder can see the application finances for organisations in the applications they assess")
    public boolean cofunderCanSeeTheApplicationFinanceForOrganisationsInApplicationsTheyAreAssignedTo(final ApplicationFinanceResource applicationFinanceResource, final UserResource user) {
        return isCofunderForApplication(applicationFinanceResource.getApplication(), user.getId());
    }
