...
public class CompetitionSearchServiceImplTest extends BaseServiceUnitTest<CompetitionSearchServiceImpl> {
    ...
    public void findProjectSetupCompetitions() {
        ...
        when(applicationRepository.findTopByCompetitionIdOrderByManageDecisionEmailDateDesc(expectedCompetitions.get(0).getId())).thenReturn(Optional.of(newApplication().withManageDecisionEmailDate(ZonedDateTime.now().minusDays(1)).build()));
        when(applicationRepository.findTopByCompetitionIdOrderByManageDecisionEmailDateDesc(expectedCompetitions.get(1).getId())).thenReturn(Optional.of(newApplication().withManageDecisionEmailDate(ZonedDateTime.now()).build()));
    ...
    public void findProjectSetupCompetitions_NoApplications() {
        ...
        when(competitionRepositoryMock.findProjectSetup(any())).thenReturn(new PageImpl<>(expectedCompetitions, PageRequest.of(page, size), 1L));
        when(applicationRepository.findTopByCompetitionIdOrderByManageDecisionEmailDateDesc(expectedCompetitions.get(0).getId())).thenReturn(Optional.empty());
