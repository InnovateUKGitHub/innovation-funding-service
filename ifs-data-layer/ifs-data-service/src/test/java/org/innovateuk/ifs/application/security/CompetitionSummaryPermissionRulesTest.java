...
public class CompetitionSummaryPermissionRulesTest extends BasePermissionRulesTest<CompetitionSummaryPermissionRules> {
    ...
    public void competitionFinanceUsersCanViewCompetitionSummaryOnAssignedComps() {
        CompetitionResource competitionResource = newCompetitionResource().build();
        List<Role> competitionFinanceRole = singletonList(EXTERNAL_FINANCE);
        ...
        when(externalFinanceRepository.existsByCompetitionIdAndUserId(competitionResource.getId(), competitionFinanceUserAssignedToCompetition.getId())).thenReturn(true);
    ...
    @Test
    public void auditorsCanViewAllCompetitionsSummary() {
        CompetitionResource competitionResource = newCompetitionResource().build();
        List<Role> auditorRoles = singletonList(AUDITOR);
        UserResource audtior = newUserResource().withRolesGlobal(auditorRoles).build();
        assertTrue(rules.auditorsCanViewAllCompetitionsSummary(competitionResource, audtior));
    }
}
