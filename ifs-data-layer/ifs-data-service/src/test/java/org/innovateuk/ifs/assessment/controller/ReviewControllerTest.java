...
public class ReviewControllerTest extends BaseControllerMockMVCTest<ReviewController> {
    ...
    public void rejectInvitation() throws Exception {
        ...
        ReviewRejectOutcomeResource reviewRejectOutcomeResource = newAssessmentReviewRejectOutcomeResource()
                .withReason(rejectComment)
                .build();

        when(assessmentPanelServiceMock.rejectAssessmentReview(assessmentReviewId, reviewRejectOutcomeResource)).thenReturn(serviceSuccess());
        ...
        verify(assessmentPanelServiceMock, only()).rejectAssessmentReview(assessmentReviewId, reviewRejectOutcomeResource);
    }
    ...
    public void getAssessmentReview() throws Exception {
        ...
        when(assessmentPanelServiceMock.getAssessmentReview(reviewResource.getId())).thenReturn(serviceSuccess(reviewResource));

        mockMvc.perform(get("/assessmentpanel/review/{id}", reviewResource.getId()))
                .andExpect(status().isOk());

        verify(assessmentPanelServiceMock, only()).getAssessmentReview(reviewResource.getId());
    }
}