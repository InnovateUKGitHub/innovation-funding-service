...
# Deploy selenium hub and chrome for local AT
alias skaffold_at="skaffold dev -f skaffold-LOCAL-AT.yml --auto-build=false --auto-sync=false --auto-deploy=false --status-check=true --wait-for-deletions=true --tail=false"
