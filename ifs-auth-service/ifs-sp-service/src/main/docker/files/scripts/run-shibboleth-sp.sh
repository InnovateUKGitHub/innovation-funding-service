...
sp_proxy_certificate=$(sed '/^-----/d' /var/certs/sp_proxy_certificate.pem | sed '{:q;N;s/\n/\\n/g;t q}')
sed -i "s#\${SP_PROXY_CERTIFICATE}#$sp_proxy_certificate#g" /etc/shibboleth/metadata.xml

idp_signing_certificate=$(sed '/^-----/d' /var/certs/idp-signing.crt | sed '{:q;N;s/\n/\\n/g;t q}')
sed -i "s#\${IDP_SIGNING_CERTIFICATE}#$idp_signing_certificate#g" /etc/shibboleth/metadata.xml

idp_encryption_certificate=$(sed '/^-----/d' /var/certs/idp-encryption.crt | sed '{:q;N;s/\n/\\n/g;t q}')
