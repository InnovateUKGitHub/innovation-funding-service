...
if test -f "$FILE"; then
    ...
    IFS_LDAP_ENCRYPTION_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/LDAP/ENCRYPTION/KEY" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_ENCRYPTION_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/ENCRYPTION/KEY" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_ENCRYPTION_CERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/ENCRYPTION/CERT" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_PROXY_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/PROXY/KEY" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_PROXY_CERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/PROXY/CERT" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_PROXY_CACERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameters-by-path --path "/CI/IFS/IDP/PROXY/CACERT" --with-decryption | jq -r ".Parameters|sort_by(.Name)[].Name")
    ...
    IFS_IDP_SIGNING_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/SIGNING/KEY" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_IDP_SIGNING_CERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/IDP/SIGNING/CERT" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_SP_PROXY_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/SP/PROXY/KEY" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_SP_PROXY_CERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/SP/PROXY/CERT" --with-decryption | jq -r ".Parameter.Value")
    ...
    IFS_SP_PROXY_CACERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameters-by-path --path "/CI/IFS/SP/PROXY/CACERT" --with-decryption | jq -r ".Parameters|sort_by(.Name)[].Name")
    ...
    IFS_SIGNED_CERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/SIGNED/CERT" --with-decryption | jq -r ".Parameter.Value")
    echo $IFS_SIGNED_CERT > server.crt
    echo "$(cat server.crt)"
    IFS_SIGNED_KEY=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameter --name "/CI/IFS/SIGNED/KEY" --with-decryption | jq -r ".Parameter.Value")
    echo $IFS_SIGNED_KEY > server.key
    echo "$(cat server.key)"
    IFS_SIGNED_CACERT=$(env AWS_PROFILE=iukorg docker run --rm -it -e AWS_PROFILE -v ~/.aws:/root/.aws ssm-access-image aws ssm get-parameters-by-path --path "/CI/IFS/SIGNED/CACERT" --with-decryption | jq -r ".Parameters|sort_by(.Name)[].Name")
