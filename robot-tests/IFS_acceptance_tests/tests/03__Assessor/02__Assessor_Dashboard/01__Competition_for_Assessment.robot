...
When the deadline has passed the assessment should not be visible
    ...
    [Teardown]    execute sql string    UPDATE `${database_name}`.`milestone` SET `DATE`='2068-01-28 00:00:00' WHERE `competition_id`='${IN_ASSESSMENT_COMPETITION}' and type = 'ASSESSOR_DEADLINE';
