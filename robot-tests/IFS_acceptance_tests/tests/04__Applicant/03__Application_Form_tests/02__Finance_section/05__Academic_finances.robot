...
Academic finances should be editable when lead marks them as complete
    ...
    Given Guest user log-in in new browser in new browser    ${test_mailbox_one}+academictest@gmail.com    ${correct_password}
...
Academic finance validations
    ...
    [Tags]
    [Setup]    Guest user log-in in new browser in new browser    ${test_mailbox_one}+academictest@gmail.com    ${correct_password}
    When the user navigates to the academic application finances
    And the user clicks the button/link    link=Your project costs
...
Lead applicant marks the finances as complete
    Guest user log-in in new browser in new browser    ${lead_applicant}    Passw0rd
