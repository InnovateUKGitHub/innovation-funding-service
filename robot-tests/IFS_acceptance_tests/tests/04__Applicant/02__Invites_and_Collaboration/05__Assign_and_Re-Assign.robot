...
Lead applicant can assign a question
    ...
    When the applicant assigns the question to the collaborator    css=#form-input-886 .editor    test1233    Dennis Bergkamp
    Then the user should see the notification    Question assigned successfully
    And the user should see the element    css=#form-input-886 .readonly
    And the question should contain the correct status/name    css=#form-input-886 .assignee span+span    Dennis Bergkamp
...
Lead applicant can assign question multiple times
    ...
    And the question should contain the correct status/name    css=#form-input-886 .assignee span+span    you
    And the applicant assigns the question to the collaborator    css=#form-input-886 .editor    test1233    Dennis Bergkamp
    Then the user should see the element    css=#form-input-886 .readonly
    And the question should contain the correct status/name    css=#form-input-886 .assignee span+span    Dennis Bergkamp
...
The question is enabled for the assignee
    ...
    And the user clicks the button/link    link= Public description
    And the user should see the element    css=#form-input-886 .editor
    And the user should not see the element    css=#form-input-886 .readonly
...
Last update message is correctly updating
    ...
    Then the question should contain the correct status/name    css=#form-input-886 .form-footer .form-footer__info    Last updated: Today by you
...
Appendices are assigned along with the question
    ...
    When the applicant assigns the question to the collaborator    css=#form-input-911 .editor    test1233    Dennis Bergkamp
...
the collaborator edits the 'public description' question
    Clear Element Text    css=#form-input-886 .editor
    Focus    css=#form-input-886 .editor
    The user enters text to a text field    css=#form-input-886 .editor    collaborator's text
    Focus    link=Sign out
    wait for autosave
...
Steve smith assigns a question to the collaborator
    ...
    When the applicant assigns the question to the collaborator  css=#form-input-886 .editor  test1233  Jessica Doe
