...
Force Tags        InnovationLead    Applicant
...
Filter ineligible applications
    ...
    Then the user should see the element    jQuery=td:contains("28")
    And the user should not see the element    jQuery=td:contains("63")
    When the user clicks the button/link    jQuery=a:contains("Clear all filters")
    Then the user should see the element       jQuery=td:contains("63")

Sort ineligible applications by lead, cannot inform user their application is ineligible
    [Documentation]    INFUND-8942, IFS-986
    [Tags]
    When the application list is sorted by    Lead
    Then the applications should be sorted by column    3
    And the user should not see the element  .button:contains("Inform applicant")

Cannot reinstate an application
   ...
   Then the user should not see the element   a:contains("Reinstate application")
