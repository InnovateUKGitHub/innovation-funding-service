...
PM can view and download the generated Grant Offer Letter
    ...
    And open pdf link                        jQuery = .grant-offer-download:contains("testing.pdf (opens in a new window)")

Other external users can view and download the uploaded Grant Offer letter
...
Non lead partner can download the annex
    ...
    [Tags]
    Then open pdf link                                         css = [href="/project-setup/project/${Elbow_Grease_Project_Id}/offer/additional-contract"]

Academic users can view and download the uploaded Grant Offer letter
    [Documentation]    INFUND-5998
    [Tags]  HappyPath
...
PM can view and download the uploaded Annex file
    [Documentation]    INFUND-4851, INFUND-4849  INFUND-5998
    [Tags]  HappyPath
    [Setup]    log in as a different user     ${Elbow_Grease_Lead_PM_Email}  ${short_password}
    ...
    And open pdf link                         css = [href="/project-setup/project/${Elbow_Grease_Project_Id}/offer/additional-contract"]
...
PM can download the signed grant offer letter
    ...
    Then open pdf link                       css = [href="/project-setup/project/${Elbow_Grease_Project_Id}/offer/signed-download"]
...
SuperAdmin user can reject and reset GOL in project setup
    ...
    Given Log in as a different user                     &{superAdminCredentials}
    When the user navigates to the page                  ${server}/project-setup-management/project/${HighSpeedProjectID}/grant-offer-letter/send
    And the user selects the radio button                REJECTED  rejectGOL
    And the user enters text to a text field             id = gol-reject-reason   Rejected
...
the user should see live project on dashboard
    ...
    the user should see the element        jQuery = li:contains("${Elbow_Grease_Title}") .status-msg:contains("Live project")
