...
the user marks every section but one as complete
    Guest user log-in in new browser    ${submit_test_email}    ${correct_password}
...
Invite and accept the invitation
    ...
    And the user clicks the button/link    jQuery=.button:contains("Sign in")
    And Guest user log-in in new browser    ${test_mailbox_one}+academictest@gmail.com    ${correct_password}
