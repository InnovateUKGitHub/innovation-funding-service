...
the user selects the checkbox
    [Arguments]    ${checkbox}
    ${status}    ${value}=    IFS Run Keyword And Ignore Error    Element Should Be Visible    xpath=//*[@id="${checkbox}" or @name="${checkbox}"]/ancestor::label[not(contains(@class,"selected"))]
...
the user unselects the checkbox
    [Arguments]    ${checkbox}
    ${status}    ${value}=    IFS Run Keyword And Ignore Error    Element Should Be Visible    xpath=//*[@id="${checkbox}" or @name="${checkbox}"]/ancestor::label[contains(@class,"selected")]
    Execute Javascript    jQuery('form label a').contents().unwrap();    # we cannot click the checkbox itself as it is hidden, however if we click the label it will click the anchor in the label, therefore I remove the <a> before submit, but keep the text
    Run Keyword If    '${status}' == 'PASS'    Click Element    xpath=//*[@id="${checkbox}" or @name="${checkbox}"]/ancestor::label
    # Error checking
    Page Should Not Contain    Error
...
the user should see the checkbox
    [Arguments]    ${checkbox}
    IFS Wait Until Element Is Visible    xpath=//*[@id="${checkbox}" or @name="${checkbox}"]/ancestor::label[contains(@class,"selected")]
    # Error checking
    Page Should Not Contain    Error
    Page Should Not Contain    something went wrong
    Page Should Not Contain    Page or resource not found
...
the user should see that the checkbox is disabled
    [Arguments]    ${checkbox}
    IFS Wait Until Element Is Visible    xpath=//*[@id="${checkbox}" or @name="${checkbox}" and @disabled='disabled']/ancestor::label[contains(@class,"selected")]
    # Error checking
    Page Should Not Contain    Error
    Page Should Not Contain    something went wrong
    Page Should Not Contain    Page or resource not found
...
the user should not see the checkbox
    [Arguments]    ${checkbox}
    IFS Wait Until Element Is Not Visible    xpath=//*[@id="${checkbox}" or @name="${checkbox}"]/ancestor::label[contains(@class,"selected")]
    # Error checking
    Page Should Not Contain    Error
    Page Should Not Contain    something went wrong
    Page Should Not Contain    Page or resource not found
...
the user sees that the radio button is selected
    ...
    IFS Wait Until element is visible    xpath=//*[@name="${RADIO_BUTTON}"][@value="${SELECTION}" or @id="${SELECTION}"]/ancestor::label[contains(@class,"selected")]
...
the user sees that the radio button is not selected
    ...
    IFS Wait Until element is visible    xpath=//*[@name="${RADIO_BUTTON}"]/ancestor::label[not(contains(@class,"selected"))]
    # Error checking
    Page Should Not Contain    Error
    Page Should Not Contain    something went wrong
    Page Should Not Contain    Page or resource not found
...
the user selects the option from the drop-down menu
    ...
    IFS Wait Until element is visible    ${drop-down}
...
Question should be editable
    ...
    ${status}    ${value}=    IFS Run Keyword And Ignore Error    Element Should Be Visible    ${Mark_question_as_incomplete}
...
The user enters text to a text field
    ...
    IFS Wait Until Keyword Succeeds    10    200ms    input text    ${TEXT_FIELD}    ${TEXT_INPUT}
    Mouse Out    ${TEXT_FIELD}
    IFS Run Keyword And Ignore Error    focus    link=Sign out
    Wait for autosave

the user sees the text in the element
    [Arguments]    ${element}    ${text}
    IFS Wait Until element is visible    ${element}
    IFS Wait Until Keyword Succeeds    10    500ms    element should contain    ${element}    ${text}
...
the user sees the text in the text field
    ...
    IFS Wait Until element is visible    ${textfield}
    IFS Wait Until Keyword Succeeds    10    200ms    textfield should contain    ${textfield}    ${text}
...
The user enters multiple strings into a text field
    ...
    IFS Wait Until Keyword Succeeds    30s    200ms    Input Text    ${field}    ${concatenated_string}
