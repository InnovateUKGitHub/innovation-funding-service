...
import org.innovateuk.ifs.util.ReflectionHelper;
...
public abstract class BaseBuilder<T, S> implements Builder<T, S> {
    ...
    public S withIdBased(BiConsumer<Long, T> multiAmendFunction) {
        BiConsumer<Integer, T> wrapperFunction = (i, t) -> {
            Long id = BaseBuilderAmendFunctions.getId(t);
    ...
    public S withArray(BiConsumer<Boolean, T> amendFunction, boolean[] values) {
        return with((i, t) -> {
            boolean nextValue = values != null && values.length > 0 ? values[Math.min(values.length - 1, i)] : null;
            amendFunction.accept(nextValue, t);
        });
    }
    ...
    public <R> S withArraySetFieldByReflection(String fieldName, R[] values) {
        return withArray((value, t) -> ReflectionHelper.setField(t, fieldName, value), values);
    }
    ...
    public <R> S withArraySetFieldByReflection(String fieldName, R[] values, String valueFieldName) {
        return withArray((value, t) -> ReflectionHelper.setField(t, fieldName, ReflectionHelper.getField
