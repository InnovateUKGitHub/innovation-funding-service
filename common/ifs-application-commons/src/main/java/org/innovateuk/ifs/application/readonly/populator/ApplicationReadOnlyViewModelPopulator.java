...
public class ApplicationReadOnlyViewModelPopulator extends AsyncAdaptor {
    ...
    public ApplicationReadOnlyViewModel populate(ApplicationResource application, CompetitionResource competition, UserResource user, ApplicationReadOnlySettings settings) {
        ...
        Future<List<QuestionStatusResource>> questionStatusesFuture = async(() -> getQuestionStatuses(application, competition, user, settings));
        ...
        return new ApplicationReadOnlyViewModel(settings,
                ...
                isLoanPartBEnabled, competition.isThirdPartyOfgem()
        );
    }
    ...
    private List<QuestionStatusResource> getQuestionStatuses(ApplicationResource application, CompetitionResource competition, UserResource user, ApplicationReadOnlySettings settings) {
        if (!settings.isIncludeStatuses() && !competition.isLoan()) {
            return emptyList();
        }
        long organisationId;
