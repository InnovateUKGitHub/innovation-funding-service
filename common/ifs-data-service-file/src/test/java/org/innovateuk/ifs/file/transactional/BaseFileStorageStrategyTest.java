...
public abstract class BaseFileStorageStrategyTest {
    ...
    @Test
    @Ignore("docker folder readonly issue")
    public void testCreateFileFailureToCreateFoldersHandledGracefully() throws IOException {
        ...
        try {
            java.nio.file.Files.setPosixFilePermissions(tempFolder.toPath(),
                    Arrays.asList(PosixFilePermission.OWNER_READ, PosixFilePermission.GROUP_READ).stream().collect(Collectors.toSet()));
            assertFalse(tempFolder.canWrite());
    ...
    public void testCreateFileFailureToCreateFileHandledGracefully() throws IOException {
        ...
        try {
            java.nio.file.Files.setPosixFilePermissions(targetFolder,
                    Arrays.asList(PosixFilePermission.OWNER_READ, PosixFilePermission.GROUP_READ).stream().collect(Collectors.toSet()));
            assertFalse(targetFolder.toFile().canWrite());

            tempFile = File.createTempFile("tempfile", "suffix", tempFolder);
